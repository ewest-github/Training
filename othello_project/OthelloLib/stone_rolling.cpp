...
int stone_rolling(int* b, int h, int w)
{
	...
	for (int f = 0; f < DIRECTION_INDEX::NUM; f++)
	{
		...
		case 0:
			...
			{
				while (w1 != 0 && *(b + ((h1 + p[DIRECTION_INDEX::LEFT].y) * HEIGHT) + w1 + p[DIRECTION_INDEX::LEFT].x) != 0 && *(b + (h * HEIGHT) + w) == *(b + ((h1 + p[DIRECTION_INDEX::LEFT].y) * HEIGHT) + w1 + p[DIRECTION_INDEX::LEFT].x))
				{
					//移動終了の判定(盤面端まで到達)
					if (w1 == 0)
					{
